...
export default function AdminDepositSettingsPage() {
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      setMessage('Deposit fallback addresses saved.');
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-[2.5rem] border border-white/70 bg-[linear-gradient(135deg,rgba(255,255,255,0.96),rgba(255,247,251,0.9))] p-6 shadow-[0_20px_80px_rgba(15,23,42,0.07)]">
        ...
        <h1 className="mt-4 text-3xl font-semibold tracking-tight text-slate-900">Manage BTC, ETH, and USDT fallback deposit addresses.</h1>
        <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-600">These addresses are shown to investors whenever a user-specific wallet address is missing or unusable. Admin login and admin auth remain unchanged.</p>
      </section>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            {CURRENCIES.map((currency) => (
              <div key={currency} className="rounded-[1.5rem] border border-slate-100 bg-slate-50/70 p-4">
                ...
                <label className="block text-sm font-medium text-slate-700">Fallback deposit address</label>
                <textarea
                  ...
                  placeholder={`Enter ${currency} fallback address`}
                />
              </div>
            ))}

            <div className="flex flex-wrap items-center justify-between gap-3">
              <div className={`text-sm ${message.includes('saved') ? 'text-emerald-700' : 'text-slate-500'}`}>{message || 'Changes apply immediately to investor deposit pages.'}</div>
              <button type="submit" disabled={saving} className="inline-flex items-center gap-2 rounded-2xl bg-slate-900 px-5 py-3 text-sm font-medium text-white transition hover:bg-slate-800 disabled:opacity-60">
                ...
                {saving ? 'Saving...' : 'Save deposit wallets'}
              </button>
            </div>
          </form>
        )}
